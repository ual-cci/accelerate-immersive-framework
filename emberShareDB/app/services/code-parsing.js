...
export default Service.extend({
  ...
  insertLibrary(lib, source) {
    ...
    let index = source.indexOf(insertBefore)
    let insert =
      '\n <script src = "' +
      config.localOrigin +
      '/libs/' +
      this.get('library').url(lib) +
      '"></script>\n'
  ...
  insertStyleSheets(source, children) {
    ...
    while ((index = source.indexOf(searchStrs[ptr], searchIndex)) > -1) {
      if (ptr == 0) {
        ...
        let added = false
        const parsedTag = this.get('parser').parseFromString(
          tag,
          'application/xml'
        )
  ...
  insertRecording(src, recordingOptions) {
    let newSrc = src
    if (
      recordingOptions.isRecording &&
      !isEmpty(recordingOptions.node.variable)
    ) {
      newSrc = ''
    ...
    scripts.forEach((script) => {
      ...
      if (!added) {
        ...
        */
        ...
        if (
          (scriptTag.includes('src=') || scriptTag.includes('src =')) &&
            ...
            node.declarations.forEach((dec) => {
              ...
              if (!isEmpty(init)) {
                if (
                  ...
                  exp.includes('maxiAudio(')
                ) {
                  possibles.push({ library: 'maximilian', variable: name })
                } else if (
                  init.type === 'NewExpression' &&
                  exp.includes('MaxiInstruments(')
                ) {
                  ...
                  init.callee.property !== undefined
                ) {
                  ...
                  if (
                    webAudioFactories.some(
                      (e) => e === init.callee.property.name
                    )
                  ) {
                    possibles.push({ library: 'WebAudio', variable: name })
                  }
  ...
  getScripts(source) {
    ...
    while ((index = source.indexOf(searchStrs[ptr], searchIndex)) > -1) {
      if (ptr == 0) {
        ...
        searchIndex = index + searchStrs[ptr].length
        const src =
          scriptStartIndex <= index - 1
            ? source.substring(scriptStartIndex, index - 1)
            : ''
        scripts.push({
          preamble: preamble,
          scriptTag: scriptTag,
    ...
    if (scripts.length > 0) {
      scripts[scripts.length - 1].post =
        scripts[scripts.length - 1].post + source.substr(prevEnd)
    }
    return scripts
  },
    ...
    return new RSVP.Promise((resolve, reject) => {
      const replaceAll = async () => {
        for (let i = 0; i < assets.length; i++) {
          ...
          if (
            this.get('assetService').isMedia(fileType) &&
            !this.get('assetService').isTooBig(assets[i].size) &&
            useBase64
          ) {
            if (!isEmpty(asset)) {
              const b64 =
                'data:' + fileType + ';charset=utf-8;base64,' + asset.b64data
  ...
  */
  ...
  getOps(delta, editor) {
    ...
    delta.forEach((change) => {
      ...
      if (
        (change.removed[0].length > 0 && change.removed.length === 1) ||
        change.removed.length > 1
      ) {
        ops.push(this.removeOp(change, editor))
      }
      if (
        (change.text[0].length > 0 && change.text.length === 1) ||
