...
define('ember-modals-manager-internal/test-support/index', ['exports', '@ember/test-helpers'], function (exports, _testHelpers) {
  'use strict';

  Object.defineProperty(exports, "__esModule", {
    value: true
  });
  exports.getConfirmButtonState = getConfirmButtonState;
  exports.getConfirmModal = getConfirmModal;
  exports.getDeclineModal = getDeclineModal;
  exports.getCustomModalText = getCustomModalText;
  exports.lastLogMessageAssert = lastLogMessageAssert;
  exports.getModalIsOpened = getModalIsOpened;
  exports.openModal = openModal;
  exports.getPromptValue = getPromptValue;
  function getConfirmButtonState(selector) {
    return function (assert, state) {
      state ? assert.ok(Ember.$(selector).is(':disabled')) : assert.ok(Ember.$(selector).is(':enabled'));
    };
  }

  function getConfirmModal(selector) {
    return async function () {
      return await (0, _testHelpers.click)(selector);
    };
  }

  function getDeclineModal(selector) {
    return async function () {
      await (0, _testHelpers.click)(selector);
    };
  }

  function getCustomModalText(selectorsMap) {
    return function (assert, part, text) {
      assert.ok((0, _testHelpers.find)(selectorsMap[part]).innerText.includes(text), `${part} has "${text}"`);
    };
  }

  async function lastLogMessageAssert(assert, msg) {
    const lastLogMsg = await (0, _testHelpers.find)('.logs').innerHTML.split('<br>').slice(0, -1).pop().trim();
    assert.equal(lastLogMsg, msg);
  }

  function getModalIsOpened(selector) {
    return async function (assert, expected) {
      if (expected) {
        const modals = await (0, _testHelpers.find)(selector);
  ...
  async function openModal(type) {
    return await (0, _testHelpers.click)(`button.${type}-modal`);
  }

  function getPromptValue(selector) {
    return async function (val) {
      await (0, _testHelpers.fillIn)(selector, val);
    };
  }
});
