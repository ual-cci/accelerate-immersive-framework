...
})()(
  {
    1: [
      function (require, module, exports) {
        module.exports = {
          ...
          homepage:
            'https://github.com/poeticAndroid/a-game/blob/master/README.md',
          main: 'index.js',
          scripts: {
            prepare: 'npm run build',
            build:
              'foreach -g src/*.js -x "browserify #{path} -o dist/#{name}.js"',
            watch:
              'foreach -g src/*.js -C -x "watchify #{path} -d -o dist/#{name}.js"',
            bump: 'npm version minor --no-git-tag-version',
            gitadd: 'git add package*.json dist/*.js',
          },
          'pre-commit': ['bump', 'build', 'gitadd'],
          keywords: ['aframe', 'aframe-component', 'webvr', 'webxr', 'gamedev'],
    ...
    2: [
      function (require, module, exports) {
        ...
        console.log(
          `${pkg.title} Version ${pkg.version} by ${pkg.author}\n(${pkg.homepage})`
        )
      },
      {
        '../package': 1,
        './components/grabbing': 3,
        './components/include': 9,
        './components/injectglove': 10,
        './components/injectplayer': 11,
        './components/limit': 12,
        './components/locomotion': 13,
        './components/onevent': 17,
        './components/onstate': 18,
        './components/physics': 19,
        './components/script': 23,
        './components/trigger': 24,
        './libs/betterRaycaster': 25,
        './libs/copyWorldPosRot': 27,
        './libs/ensureElement': 28,
        './libs/pools': 29,
        './libs/touchGestures': 30,
        './primitives/a-glove': 31,
        './primitives/a-hand': 32,
        './primitives/a-main': 33,
        './primitives/a-player': 34,
      },
    ],
    3: [
      function (require, module, exports) {
        ...
        AFRAME.registerComponent('grabbing', {
          ...
          init() {
            ...
            for (let hand of this._hands) {
              let _hand = '_' + hand
              this[_hand].hand.addEventListener(
                'buttonchanged',
                this._enableHands
              )
            }

            this._head.ray = this._head.hand.ensure(
              '.grabbing-ray',
              'a-entity',
              {
                class: 'grabbing-ray',
            ...
            this._head.buttonRay = this._head.hand.ensure(
              '.button.ray',
              'a-entity',
              {
                class: 'button ray',
            ...
            this._head.anchor = this._head.ray.ensure(
              '.grabbing.anchor',
              'a-entity',
              {
            ...
            for (let hand of this._hands) {
              ...
              for (let finger = 0; finger < 5; finger++) {
                this.emit(
                  ...
                  this[_hand].glove,
                  this[_hand].grabbed,
                  { hand: hand, finger: finger, flex: flex }
                )
              }
            }
          },
          ...
          tick(time, timeDelta) {
            ...
            if (
              (this._gamepadBtns[4] || this._gamepadBtns[5]) &&
              !this._grabBtn
            ) {
            ...
            if (
              (this._gamepadBtns[6] || this._gamepadBtns[7]) &&
              !this._useBtn0
            )
              this.useDown()
            ...
            for (let hand of this._hands) {
              ...
              if (this.data.avoidWalls && this[_hand]._occlusionRay) {
                let palm =
                  this[_hand].glove.querySelector('.palm') || this[_hand].glove
                this[_hand].glove.copyWorldPosRot(this[_hand].hand)
              ...
              if (this[_hand].grabbed) {
                ...
                if (!this[_hand].grabbed.components.grabbable?.data.immovable) {
                  if (
                    this[_hand].grabbed.components.grabbable?.data.avoidWalls
                  ) {
              ...
              this[_hand].lastGlovePos =
                this[_hand].lastGlovePos || new THREE.Vector3()
          ...
          _enableHands() {
            ...
            for (let hand of this._hands) {
              let _hand = '_' + hand
              this[_hand].hand.removeEventListener(
                'buttonchanged',
                this._enableHands
              )
              ...
              this[_hand].ray = palm.ensure('.grabbing.ray', 'a-entity', {
                class: 'grabbing ray',
                position:
                  hand === 'left' ? '-0.0625 0 0.0625' : '0.0625 0 0.0625',
          ...
          _onButtonChanged(e) {
            ...
            switch (e.detail.id) {
              case 0: // Trigger
                finger = 1
                if (
                  e.detail.state.pressed &&
                  !this._btnPress[hand + e.detail.id]
                )
                  this.useDown(hand)
                if (
                  !e.detail.state.pressed &&
      ...
      {
        './grabbing/button': 4,
        './grabbing/climbable': 5,
        './grabbing/fingerflex': 6,
        './grabbing/grabbable': 7,
        './grabbing/receptacle': 8,
      },
    ],
    4: [
      function (require, module, exports) {
        ...
        AFRAME.registerComponent('climbable', {
          ...
          init() {
            this.el.setAttribute(
              'grabbable',
              'physics:false; kinematicGrab:false; immovable:true;'
            )
          ...
          events: {
            grab(e) {
              if (currentClimb && currentClimb !== this.el)
                this._player.components.grabbing.dropObject(currentClimb)
              ...
              if (this._handName === 'head') {
                ...
                this._hand.object3D.position.set(
                  0,
                  0,
                  -e.detail.intersection.distance
                )
              }
              ...
              setTimeout(() => {
                this.el.sceneEl
                  ...
                  ?.object3D.position.copy(
                    this._player.components.locomotion.headPos
                  )
              })
        ...
        AFRAME.registerComponent('fingerflex', {
          ...
          tick(time, timeDelta) {
            for (let finger = 0; finger < 5; finger++) {
              ...
              current =
                current + Math.random() * Math.random() * (target - current)
              let degrees =
                this.data.min + current * (this.data.max - this.data.min)
    ...
    7: [
      function (require, module, exports) {
        ...
        AFRAME.registerComponent('grabbable', {
          ...
          init() {
            if (this.data.physics && !this.el.hasAttribute('body'))
          ...
          events: {
            ...
            limited(e) {
              if (this._grabbed) {
                ...
                this._glove.object3D.updateMatrixWorld(true, true)
      ...
      function (require, module, exports) {
        ...
        AFRAME.registerComponent('receptacle', {
          ...
          tick() {
            ...
            if (this._lastNearest && this._lastNearest !== this.nearest) {
              ...
              if (!this._hover) {
                ...
                if (this.data.autoDrop && this._grabber)
                  this._grabber.dropObject(this.nearest)
              }
              this._anchor.removeAttribute('animation__pos')
              this._anchor.removeAttribute('animation__rot')
              this._anchor.copyWorldPosRot(this.nearest)
              this._hover = true
              if (this.nearest.is('grabbed')) this._grabbed = true
            } else if (this._grabbed || !this.data.onlyGrabbed) {
              if (!this.el.is('occupied')) {
                this._anchor.copyWorldPosRot(this.nearest)
                this._anchor.components.body.commit()
                if (this.nearest.components.body)
                  this._anchor.setAttribute('joint__put', {
                    body2: this.nearest,
                    type: 'lock',
                  })
          ...
          refreshObjects() {
            ...
            this._grabber =
              this.el.sceneEl.querySelector('[grabbing]')?.components.grabbing
          },
        })
      },
      {},
          ...
          async init() {
            if (this.data && !this.el.sceneEl._including_) {
              ...
              if (response.status >= 200 && response.status < 300) {
                this.el.outerHTML = await (await response.text())
                  .replace('>', ' >')
                  .replace(' ', ' ' + attrs + ' ')
              } else {
              ...
              if (next && next.components && next.components.include)
                next.components.include.init()
            }
          },
        })
      },
      ...
      function (require, module, exports) {
        ...
        AFRAME.registerComponent('injectglove', {
          init() {
            ...
            let hand =
              this.el.getAttribute('side') ||
              this.el.parentNode.getAttribute('side')
          ...
          defaultGlove() {
            ...
            let color =
              ...
              this.el.parentNode.getAttribute('color') ||
              'lightblue'
            if (!this.el.getAttribute('fingerflex'))
              this.el.setAttribute('fingerflex', {
                min: hand === 'left' ? -10 : 10,
                max: hand === 'left' ? -90 : 90,
              })
            this.el.innerHTML = `<a-box class="palm" color="${color}" position="${
              hand === 'left' ? -0.01 : 0.01
            } -0.03 0.08" rotation="-35 0 0" width="0.02" height="0.08"
      depth="0.08">
      <a-entity position="0 0.04 0.02" rotation="80 0 ${
        hand === 'left' ? -45 : 45
      }">
        ...
        AFRAME.registerComponent('locomotion', {
          ...
          init() {
            ...
            this._legs = this.el.sceneEl.ensure('.legs', 'a-entity', {
              class: 'legs',
              position: '0 0.5 0', // radius: 0.125, color: "blue",
              raycaster: {
                deep: true,
                autoRefresh: false,
                objects: '[floor]',
            ...
            this._legBumper = this.el.sceneEl.ensure(
              '.leg-bumper',
              'a-entity',
              {
                class: 'leg-bumper',
                position: '0 0.5 0', // radius: 0.125, color: "red",
                raycaster: {
                  deep: true,
                  autoRefresh: false,
                  objects: '[wall]',
            ...
            this._headBumper = this.el.sceneEl.ensure(
              '.head-bumper',
              'a-entity',
              {
                class: 'head-bumper',
                position: '0 0.5 0', // radius: 0.125, color: "green",
                raycaster: {
                  deep: true,
                  autoRefresh: false,
                  objects: '[wall]',
            ...
            this._teleportBeam = this._camera.ensure(
              '.teleport-ray',
              'a-entity',
              {
                class: 'teleport-ray',
            ...
            this._teleportCursor = this.el.ensure(
              '.teleport-cursor',
              'a-cylinder',
              {
                class: 'teleport-cursor',
                radius: 0.5,
                height: 0.0625,
          ...
          play() {
            ...
            this._leftHand.addEventListener(
              'buttonchanged',
              this._onButtonChanged
            )
            this._rightHand.addEventListener(
              'buttonchanged',
              this._onButtonChanged
            )
            this.el.sceneEl.canvas.addEventListener(
          ...
          pause() {
            ...
            this._leftHand.removeEventListener(
              'buttonchanged',
              this._onButtonChanged
            )
            this._rightHand.removeEventListener(
              'buttonchanged',
              this._onButtonChanged
            )
            this.el.sceneEl.canvas.removeEventListener(
          ...
          tick(time, timeDelta) {
            ...
            this.headDir
              ...
              .applyQuaternion(
                this.el.object3D.getWorldQuaternion(THREE.Quaternion.temp())
              )
            ...
            if (head2toe.length() > 0.5 || !this.currentFloor) {
              if (this.currentFloor) head2toe.multiplyScalar(0.1)
            ...
            if (!this._godMode && !this._caution) {
              ...
              if (hit && this._vertVelocity <= 0) {
                ...
                this.currentFloorPosition.copy(
                  this.currentFloor.object3D.position
                )
              } else {
                if (this.currentFloor) this.currentFloor.emit('leave')
                this._vertVelocity -= this.data.gravity * timeDelta
                this.move(
                  THREE.Vector3.temp().set(
                    0,
                    Math.max(-0.5, this._vertVelocity * timeDelta),
                    0
                  )
                )
            ...
            // bump walls
            if (this._godMode) {
              this._legBumper.object3D.position.copy(
                this._legs.object3D.position
              )
              this._headBumper.object3D.position.copy(
                this._legs.object3D.position
              )
            } else if (
              this._bumpOverload > 4 ||
              Math.abs(this.headPos.y - this.feetPos.y) > 3
            ) {
          ...
          _bump(pos, bumper) {
            ...
            if (dist) {
              ...
              if (hit) {
                ...
                this._legs.object3D.position.add(delta)
                this._legs.object3D.position.y = Math.max(
                  feety,
                  this.headPos.y - 1.5
                )
                this._caution = 4
          ...
          _callMoveStick() {
            ...
            if (this._kbStick.length() > 0.1)
              this._kbStick.multiplyScalar(
                (this._kbStick.length() - 0.1) / this._kbStick.length()
              )
            else this._kbStick.set(0, 0)
            this._kbStick.add(stick.multiplyScalar(0.2))
            if (this._kbStick.length() > 1) this._kbStick.normalize()
            if (this._kbStick.length() > bestStick.length())
              bestStick.copy(this._kbStick)
            ...
            if (this._keysDown['ShiftLeft'] || this._keysDown['ShiftRight'])
              bestStick.multiplyScalar(0.25)
            return bestStick
          },
          _applyMoveStick(seconds) {
            ...
            let heading =
              THREE.Vector2.temp()
                .set(this.headDir.z, -this.headDir.x)
                .angle() - Math.PI
          ...
          _applyAuxStick(seconds) {
            ...
            if (this._godMode) {
              this.el.object3D.position.y +=
                -stick.y * this.data.speed * seconds
              this._legs.object3D.position.y +=
                -stick.y * this.data.speed * seconds
            } else {
              // Crouching
              ...
              if (Math.round(stick.y) < 0) {
                ...
                this._teleportCursor.object3D.quaternion
                  .multiply(quat.conjugate().normalize())
                  .multiply(
                    quat
                      .copy(this.el.object3D.quaternion)
                      .multiply(this._camera.object3D.quaternion)
                  )
                ...
                if (hit && hit.el.components.floor) {
                  ...
                  if (delta.length() > this.data.teleportDistance)
                  ...
                  this._teleportCursor.object3D.parent.worldToLocal(
                    this._teleportCursor.object3D.position
                  )

                  matrix.getNormalMatrix(hit.el.object3D.matrixWorld)
                  delta.copy(hit.face.normal).applyMatrix3(matrix).normalize()
                  delta.applyQuaternion(
                    quat.copy(this.el.object3D.quaternion).conjugate()
                  )
                  straight.set(0, 1, 0)
          ...
          _applyButtons() {
            ...
            if (buttons) {
              if (!this._toggling) {
                ...
                if (this.data.godMode && buttons & 2)
          ...
          _deadZone(vec, limit = 0.25) {
            if (vec.length() > limit) {
              vec.multiplyScalar(
                (vec.length() - limit) / (1 - limit) / vec.length()
              )
            } else {
              vec.set(0, 0)
            }
          ...
          _onAxisMove(e) {
            ...
            if (!this._handEnabled) {
              ...
              this._teleportBeam = this._rightHand.ensure(
                '.teleportBeam',
                'a-entity',
                {
                  class: 'teleportBeam',
            ...
            for (let j = 0; j < e.changedTouches.length; j++) {
              ...
              if (touchDir) {
                ...
                if (touchDir.length() > stickRadius) {
                  touchDir.multiplyScalar(
                    (touchDir.length() - stickRadius) / touchDir.length()
                  )
    ...
    15: [
      function (require, module, exports) {
        ...
        AFRAME.registerComponent('start', {
          init() {
            let loco =
              this.el.sceneEl.querySelector('[locomotion]').components
                .locomotion
            if (!loco)
              return setTimeout(() => {
                this.init()
      ...
      function (require, module, exports) {
        ...
        AFRAME.registerComponent('onevent', {
          ...
          update(oldData) {
            ...
            if (this.el.isPlaying) this.play()
          },
      ...
      function (require, module, exports) {
        ...
        AFRAME.registerSystem('physics', {
          schema: {
            workerUrl: {
              type: 'string',
              default: `https://cdn.jsdelivr.net/npm/a-game@${pkg.version}/dist/cannonWorker.min.js`,
            },
            ...
            if (this.data.workerUrl) {
              if (!this.worker) {
                if (this.data.workerUrl.includes('//')) {
                  let script = `importScripts(${JSON.stringify(
                    this.data.workerUrl
                  )})`
                ...
                this.worker.postMessage(
                  'log ' + cmd.stringifyParam('Physics worker ready!')
                )
              ...
              this.worker.postMessage(
                'world gravity = ' + cmd.stringifyParam(this.data.gravity)
              )
          ...
          onMessage(e) {
            if (typeof e.data === 'string') {
              ...
              while (this.buffers.length > 2) this.buffers.shift()
            }
          },
          ...
          command(params) {
            ...
            switch (params.shift()) {
              case 'body':
                ...
                if (body) body.components.body.command(params)
                break
            }
          },
          eval(expr) {
      ...
      function (require, module, exports) {
        ...
        AFRAME.registerComponent('body', {
          ...
          init() {
            ...
            body.position = this.el.object3D.localToWorld(
              THREE.Vector3.temp().set(0, 0, 0)
            )
            body.quaternion = this.el.object3D.getWorldQuaternion(
              THREE.Quaternion.temp()
            )
            if (this.mid !== null) {
              let p = this.mid * 8
            ...
            this.sleeping = true
            worker.postMessage(
              'world body ' + this.id + ' create ' + cmd.stringifyParam(body)
            )
            // if (body.type === "static")
            setTimeout(() => {
              body.position = this.el.object3D.localToWorld(
              ...
              worker.postMessage(
                'world body ' +
                  this.id +
                  ' position = ' +
                  cmd.stringifyParam(body.position)
              )
              worker.postMessage(
                'world body ' +
                  this.id +
                  ' quaternion = ' +
                  cmd.stringifyParam(body.quaternion)
              )
              ...
              for (let comp in this.el.components) {
                if (comp.substr(0, 7) === 'joint__')
                  this.el.components[comp].play()
              }
            })
            ...
            if (this.data.type !== oldData.type)
              worker.postMessage(
                'world body ' +
                  this.id +
                  ' type = ' +
                  cmd.stringifyParam(this.data.type)
              )
            if (this.data.mass !== oldData.mass)
              worker.postMessage(
                'world body ' +
                  this.id +
                  ' mass = ' +
                  cmd.stringifyParam(this.data.mass)
              )
            if (this.data.friction !== oldData.friction)
              worker.postMessage(
                'world body ' +
                  this.id +
                  ' friction = ' +
                  cmd.stringifyParam(this.data.friction)
              )
            if (this.data.restitution !== oldData.restitution)
              worker.postMessage(
                'world body ' +
                  this.id +
                  ' restitution = ' +
                  cmd.stringifyParam(this.data.restitution)
              )
            if (this.data.belongsTo !== oldData.belongsTo)
              worker.postMessage(
                'world body ' +
                  this.id +
                  ' belongsTo = ' +
                  cmd.stringifyParam(this.data.belongsTo)
              )
            if (this.data.collidesWith !== oldData.collidesWith)
              worker.postMessage(
                'world body ' +
                  this.id +
                  ' collidesWith = ' +
                  cmd.stringifyParam(this.data.collidesWith)
              )
            if (this.data.emitsWith !== oldData.emitsWith)
              worker.postMessage(
                'world body ' +
                  this.id +
                  ' emitsWith = ' +
                  cmd.stringifyParam(this.data.emitsWith)
              )
            // if (this.data.sleeping !== oldData.sleeping)
            setTimeout(() => {
              worker.postMessage(
                'world body ' + this.id + ' sleeping = ' + !!this.data.sleeping
              )
            })
          },
          ...
          applyWorldImpulse(force, point) {
            ...
            worker.postMessage(
              'world body ' +
                this.id +
                ' impulse ' +
                cmd.stringifyParam(force) +
                ' ' +
                cmd.stringifyParam(point)
            )
          },
          applyLocalImpulse(force, point) {
            let _point = this.el.object3D.localToWorld(
              THREE.Vector3.temp().copy(point)
            )
            let _force = this.el.object3D
              .localToWorld(THREE.Vector3.temp().copy(force))
              .sub(
                this.el.object3D.localToWorld(THREE.Vector3.temp().set(0, 0, 0))
              )
          ...
          pause() {
            ...
            bodies[this.id] = null
            if (this.mid !== null) movingBodies[this.mid] = null
          ...
          tick() {
            ...
            if (this.mid !== null) {
              ...
              if (this.data.type === 'kinematic') {
                let vec = this.el.object3D.localToWorld(
                  THREE.Vector3.temp().set(0, 0, 0)
                )
                buffer[p++] = vec.x
                buffer[p++] = vec.y
                ...
                let quat = this.el.object3D.getWorldQuaternion(
                  THREE.Quaternion.temp()
                )
                buffer[p++] = quat.x
                buffer[p++] = quat.y
          ...
          eval(expr) {
            let worker = this.el.sceneEl.systems.physics.worker
            worker.postMessage(
              'world body ' + this.id + ' eval ' + cmd.stringifyParam(expr)
            )
          },
          ...
          commit() {
            ...
            this.el.object3D.localToWorld(pos.set(0, 0, 0))
            worker.postMessage(
              'world body ' + this.id + ' position ' + cmd.stringifyParam(pos)
            )
            this.el.object3D.getWorldQuaternion(quat)
            worker.postMessage(
              'world body ' +
      ...
      function (require, module, exports) {
        ...
        AFRAME.registerComponent('joint', {
          ...
          play() {
            ...
            if (!worker) return
            if (!this.data.body1.components.body)
              return (this._retry = setTimeout(() => {
                this.play()
              }, 256))
            if (!this.data.body2.components.body)
              return (this._retry = setTimeout(() => {
                this.play()
              }, 256))
            ...
            joint.body1 = this.data.body1
              ? this.data.body1.components.body.id
              : this.el.components.body.id
            ...
            worker.postMessage(
              'world joint ' + this._id + ' create ' + cmd.stringifyParam(joint)
            )
            // })
          },
          ...
          eval(expr) {
            let worker = this.el.sceneEl.systems.physics.worker
            worker.postMessage(
              'world joint ' + this._id + ' eval ' + cmd.stringifyParam(expr)
            )
          },
        })
      },
      ...
      function (require, module, exports) {
        ...
        AFRAME.registerComponent('shape', {
          ...
          play() {
            ...
            this.body = this.el
            while (this.body && !this.body.matches('[body]'))
              this.body = this.body.parentElement
            if (!this.body)
              return (this._retry = setTimeout(() => {
                this.play()
              }, 256))
            ...
            let shape = {}
            shape.position = this.el.object3D.getWorldPosition(
              THREE.Vector3.temp()
            )
            this.body.object3D.worldToLocal(shape.position)
            shape.quaternion = this.el.object3D.getWorldQuaternion(
              THREE.Quaternion.temp()
            )
            let bodyquat = this.body.object3D.getWorldQuaternion(
              THREE.Quaternion.temp()
            )
            shape.quaternion
              .multiply(bodyquat.conjugate().normalize())
              .normalize()
            ...
            switch (this.el.tagName.toLowerCase()) {
              case 'a-sphere':
                shape.type = 'sphere'
                shape.size.multiplyScalar(
                  this.el.components.geometry.data.radius * 2
                )
                break
              case 'a-cylinder':
                shape.type = 'cylinder'
                shape.size.multiplyScalar(
                  this.el.components.geometry.data.radius * 2
                ).y = this.el.components.geometry.data.height
                break
              case 'a-box':
                shape.type = 'box'
                shape.size.set(
          ...
          pause() {
            ...
            let shapes = this.body.components.body.shapes
            worker.postMessage(
              'world body ' + this.bodyId + ' shape ' + this.id + ' remove'
            )
            shapes[this.id] = null
            this.id = null
          },
        ...
        AFRAME.registerComponent('script', {
          ...
          async update(oldData) {
            if (this.data.src !== oldData.src) {
              if (this.script) {
                if (this.el.isPlaying) this.script.pause?.()
                this.script.remove?.()
              }
              this.script = null
              ...
              if (response.status >= 200 && response.status < 300) {
                ...
                if (this.script.events) {
                  for (let event in this.script.events) {
                    this.script.events[event] = this.script.events[event].bind(
                      this.script
                    )
                  }
                }
              ...
              this.script.init?.()
              if (this.el.isPlaying) this.script.play?.()
            }
          ...
          tick() {
            ...
            for (let obj of this.objects) {
              ...
              if (!inside && this.triggered.indexOf(obj) >= 0) {
                ...
                if (!this.triggered.length) this.el.removeState('triggered')
              }
            }
          },
    ...
    25: [
      function (require, module, exports) {
        ...
        const _refreshObjects =
          AFRAME.components.raycaster.Component.prototype.refreshObjects
        AFRAME.components.raycaster.schema.deep =
          AFRAME.components.raycaster.schema.showLine
          ...
          function () {
            ...
            if (this.data.deep) {
              ...
              for (let hit of hits) {
                ...
                while (hit.el && !hit.el.matches(this._matchSelector))
                  hit.el = hit.el.parentNode
              }
            }
            return result
          }
    ...
    27: [
      function (require, module, exports) {
        ...
        AFRAME.AEntity.prototype.copyWorldPosRot = function (srcEl) {
          ...
          dest.updateMatrixWorld(true, true)
        }
      },
      {},
    ],
    28: [
      function (require, module, exports) {
        Element.prototype.ensure = function (
          selector,
          name = selector,
          attrs = {},
          innerHTML = ''
      ...
      function (require, module, exports) {
        ...
        function makePool(Class) {
          ...
          Class.temp = function () {
            ...
            Class._inUse.push(v)
            if (!Class._gc) Class._gc = setTimeout(Class._recycle)
            return v
          }
          Class._recycle = function () {
            while (Class._inUse.length) Class._pool.push(Class._inUse.pop())
            Class._gc = false
          }
        }
